    ...
    <Controller
      ...
      render={({ field }) => (
        <Autocomplete
          ...
          {...(multiple && {
            renderTags: (value) => Array.isArray(value) && value.map((option, index) => <SubjectChip key={index} Subject={option as SubjectType} />),
...
function YearGroupAutocomplete({ multiple, sx, initialValue }: { multiple?: boolean; sx?: SxProps<Theme>; initialValue?: YearGroupType }) {
  const { control } = useFormContext();
  return (
    <Controller
      name="year_group"
      ...
      render={({ field }) => (
        <Autocomplete
          ...
          renderInput={(params) => <TextField {...params} variant="outlined" label="Year Group" placeholder="Select year group" />}
          sx={sx}
        />
      )}
    />
    ...
    <Controller
      ...
      render={({ field }) => (
        <Autocomplete
          ...
          renderInput={(params) => <TextField {...params} variant="outlined" label="Communication" placeholder="Select communication medium" />}
          sx={sx}
        />
      )}
    />
