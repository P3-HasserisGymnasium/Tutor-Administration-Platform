...
function obtainValidTimeAvailabilities(
  ...
  const existingSameDayTimeAvailability = existingTimeAvailabilities.find((time: TimeAvailabilityType) => time.day === newTimeAvailability.day);
  ...
  const equivalentTimeSlot = existingSameDayTimeAvailability?.time.find(
    (timeslot) => timeslot.start_time === newTimeAvailability.time[0].start_time && timeslot.end_time === newTimeAvailability.time[0].end_time
  );
