...
export default function PostCreation(){
    ...
    return (
        <FormProvider {...filterMethods}>
            <Box sx={{padding:"1em", display:"flex", flexDirection:"column", height:"94%", justifyContent:"space-between"}}>
                <Box sx={{display:"flex", flexDirection:"column", gap:"1em"}}>
                    <Typography variant="h2">Your Post</Typography>
                    <Typography variant="h3">Choose title</Typography>
                    <TextField variant="outlined" size="small" placeholder="Insert title" {...register("title")} sx={{ minWidth: "50%", maxWidth: "100%", width: "fit-content",}}/>
                    <CustomAutocomplete variant="subject" sx={{width:"50%"}}/>
                    <Box sx={{ display: "flex", alignItems: "flex-start", flexDirection:"column", gap: 0 }}>
                        {!checked && <Box sx={{width:"100%"}}> <SetDuration/></Box>}
                ...
                <Box sx={{ display: "flex", justifyContent: "flex-end", alignItems:"center"}}>
                    <Button variant="contained" onClick={filterMethods.handleSubmit(createPost)} disabled={!(getValues("subject") && getValues("title"))}>Create post</Button>
                </Box>
            </Box>
        </FormProvider>
    )
